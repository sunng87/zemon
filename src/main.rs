...
fn main() {
    ...
    loop {
        ...
        for (i, cpu) in cpus.iter().enumerate() {
            mvprintw(5 + i as i32, 2, &format!("CPU {}: {:.1}%", i, cpu.cpu_usage()));
        }
        
        // Display memory information
        let memory_row = 6 + cpus.len() as i32;
        mvprintw(memory_row, 0, "Memory Usage:");
        mvprintw(memory_row + 1, 2, &format!("Total: {:.2} GB", system.total_memory() as f64 / 1024.0 / 1024.0 / 1024.0));
        mvprintw(memory_row + 2, 2, &format!("Used:  {:.2} GB", system.used_memory() as f64 / 1024.0 / 1024.0 / 1024.0));
        mvprintw(memory_row + 3, 2, &format!("Free:  {:.2} GB", system.free_memory() as f64 / 1024.0 / 1024.0 / 1024.0));
